...
/*   Updated: 2021/02/08 16:14:13 by ctaleb           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int		wall_check(float x, float y, t_mlx_params *mlx)
{
	...
	c_x = (int)x;
	c_y = (int)y;
	printf("X CAST = %i | Y CAST = %i | C = %c\n", c_x, c_y, mlx->map->grid[c_y][c_x]);
	if (mlx->pl->cam_x >= 0 && mlx->pl->cam_y <= 0)
	{
		if ((x >= mlx->pl->nwall_x && y <= mlx->pl->nwall_y) || (mlx->map->grid[c_y][c_x] == '1'))
			return (0);
	}
